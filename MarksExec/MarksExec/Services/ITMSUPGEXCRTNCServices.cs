        ...
        private static bool InsertToSql(List<ITMSUPGEXCRTNC> iTMSUPs)
        {
            ...
            using (conn)
            {
                ...
                using (var transaction = conn.BeginTransaction())
                {
                    try
                    {
                        string strsql = "Insert into  ITMSUPGEXCRTNC_TMP " +
                              "values(@DC,@dpno,@fullcode,@SupplierCode,@stopreasoncode"
                                   + ",@Cname,@mainsupp,@realsupp,@suppCname,@typeofsales"
                                   + ",@Grade,@spec,@seasoncode,@Permanentstop,@Supstop"
                                   + ",@Supstopdate,@Goodexchangeflag,@OnlyBreakage"
                                   + ",@Venderneedcheckproduct,@Goodexchangeflagforstopitem"
                                   + ",@OnlyBreakage1,@Venderneedcheckproduct1,@ReturnFlagNonStopItem"
                                   + ",@NoReturn,@AcceptBreakage,@OnlyGoodProduct,@OnlyBreakage_incl"
                                   + ",@OnlyBreakage_ProductDefective,@OnlyIntactStockProduct,@MDSpecialReutrn"
                                   + ",@SupplierSpecialDemands,@VenderNeedCheckProduct2,@CompleteOriginalAccessaryDocumentA"
                                   + ",@OriginalCoverBox,@OriginalProductPackage,@Return_Flag_TemporaryStopItem"
                                   + ",@NoReturn1,@Accept_BreakageandGood_Product1,@OnlyGoodProduct1,@OnlyBreakage_Near_Over_ExpireDat1"
                                   + ",@OnlyBreakage_ProductDefective1,@OnlyIntactStockProduct1,@MDSpecialReutrn1,@SupplierSpecialDemands_RefertoLogis1"
                                   + ",@VenderNeedCheckProduct3,@CompleteOriginalAccessaryDocument,@OriginalCoverBox1,@OriginalProductPackage1"
                                   + ",@ReturnFlag_PermanentStopItem,@NoReturn2,@AcceptBreakageandGoodProduct2,@OnlyGoodProduct2"
                                   + ",@OnlyBreakage_Near_Over_ExpireDat2,@OnlyBreakage_ProductDefective2,@OnlyIntactStockProduct2"
                                   + ",@MDSpecialReutrn2,@SupplierSpecialDemandsRefertoLogis2,@VenderNeedCheckProduct4"
                                   + ",@CompleteOriginalAccessaryDocumentA2,@OriginalCoverBox2,@OriginalProductPackage2)";
                                




                        conn.Execute(strsql, iTMSUPs, transaction);
